...
bool ArithmeticToGEPPass::runOnFunction(llvm::Function &F) {
  ...
  LocalValue<true> v1{ std::declval<llvm::Instruction *>() };
  LocalValue<true> v2{ std::declval<const llvm::Instruction *>() };
  LocalValue<false> v3{ std::declval<llvm::Argument *>() };
  // The following does not compile because const llvm::Argument cannot be use
  // to initialize a mutable LocalValue
  // LocalValue<false> v4{std::declval<const llvm::Argument*>()};

  // These work because we have the assignment operator from DerivedFromLocal *
  v1 = std::declval<llvm::Instruction *>();
  v1 = std::declval<const llvm::Instruction *>();
  ...
  // v3 = std::declval<llvm::ConstantInt *>();
  // The following does not compile because const llvm::Instruction cannot be
  // use to initialize a mutable LocalValue
  // v3 = std::declval<const llvm::Instruction *>();

  LocalValue w1{ std::declval<llvm::Instruction *>() };
  LocalValue w2{ std::declval<const llvm::Instruction *>() };

  // This works because we have the assignment operator
  w2 = std::declval<llvm::Instruction *>();
  w2 = std::declval<const llvm::Instruction *>();
  w1 = std::declval<llvm::Instruction *>();
  // The following doesn't compile because we can't assign a const
  // llvm::Instruction to a mutable LocalValue
  // w1 = std::declval<const llvm::Instruction *>();
  ...
  LocalValue<true> gv1{ std::declval<llvm::Use *>() };
  LocalValue<true> gv2{ std::declval<const llvm::Use *>() };
  LocalValue<false> gv3{ std::declval<llvm::Use *>() };
  // The following does not compile because const llvm::Use cannot be use
  // to initialize a mutable LocalValue
  // LocalValue<false> v4{std::declval<const llvm::Use*>()};
  LocalValue gw1{ std::declval<llvm::Use *>() };
  LocalValue gw2{ std::declval<const llvm::Use *>() };

  // This works because we have the assignment operator
  gw2 = std::declval<llvm::Use *>();
  gw2 = std::declval<const llvm::Use *>();
  gw1 = std::declval<llvm::Use *>();
  // The following doesn't compile because we can't assign a const
  // llvm::Instruction to a mutable LocalValue
  // gw1 = std::declval<const llvm::Use *>();
