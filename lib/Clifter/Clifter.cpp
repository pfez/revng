...
private:
  ...
  emitExpression(const llvm::Value *V, mlir::Location SurroundingLocation) {
    ...
    if (auto *GEP = llvm::dyn_cast<llvm::GetElementPtrInst>(V)) {
      ...
      mlir::Location Loc = getLocation(GEP);

      if (GEP->getNumIndices() == 1
          and GEP->getSourceElementType()->isIntegerTy(8)) {

        using PrimitiveKind::UnsignedKind;
        auto PointerType = makePointerType(getPrimitiveType(1, UnsignedKind));
        ...
        auto Cmp = getIntptrType().getByteSize() <=> IndexType.getByteSize();
        if (Cmp < 0)
          Index = emitCast(Loc, Index, getIntptrType(), CastKind::Truncate);
        else if (Cmp > 0)
          Index = emitCast(Loc, Index, getIntptrType(), CastKind::Extend);
