...
struct model::QualifiedType {
  TypePath UnqualifiedType = makeTypePath(nullptr);
  std::vector<Qualifier> Qualifiers = {};
  // UnqualifiedType is valid, all Qualifiers are valid, and there are no
  // consecutive Const entry in Qualifiers.
  bool verify() const debug_function;
...
inline bool isQualifiedType(const QualifiedType &QT) {
  // FIXME: should we unwrap UnqualifiedType and look for nested qualifiers?
  revng_assert(QT.verify());
  return not QT.Qualifiers.empty();
}

extern uint64_t typeSize(const model::Type *T);
extern uint64_t typeSize(const model::QualifiedType &T);
...
public:
  ...
  // Kind must be TypeKind::Enum; Underlying type must verify and be an
  // integral primitive type; there must be at least one entry, and all the
  // entries must verify.
  bool verify() const debug_function;
...
struct model::TypedefType : public model::Type {
  ...
  // Kind must be TypeKind::Typedef, Underlying type must verify, and
  // Underlying type must not refer directly to this type
  bool verify() const debug_function;
...
struct model::StructType : public model::Type {
  ...
  // Kind must be TypeKind::Struct, Fields must not be empty, all Fields must
  // verify and Fields must not have duplicate names.
  bool verify() const debug_function;
...
struct model::UnionType : public model::Type {
  ...
  // Kind must be TypeKind::Union, Fields must not be empty, all Fields must
  // verify
  bool verify() const debug_function;
...
struct model::FunctionPointerType : public model::Type {
  ...
  // Kind must be TypeKind::FunctionPointer; ReturnType and all the
  // ArgumentTypes must verify; ArgumentTypes may contain the void type only if
  // there is one argument type and the function is not variadic
  bool verify() const debug_function;
...
inline bool verifyType(const model::UpcastableType &T) {
  const auto Verify = [](auto &Upcastable) { return Upcastable.verify(); };
  return upcast<bool>(T, Verify, false);
}

inline bool verifyType(const model::Type *T) {
  const auto Verify = [](auto &Upcastable) { return Upcastable.verify(); };
  return upcast<bool>(T, Verify, false);
}
