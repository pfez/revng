...
#include "revng/RestructureCFG/MarkerCallUtils.h"
...
public:
  ...
  static void verify(NodeRef N) {
    verifyBasicBlock(FunctionTags::ScopeCloserMarker, N);
    verifyBasicBlock(FunctionTags::GotoBlockMarker, N);
  }
};
...
struct llvm::DOTGraphTraits<Scope<llvm::Function *>>
  ...
  //       print:
  //       - The scope closer edges should be markerd with a different color
  //       - The CFG edges that are ignored due to the fact that they are
  //         cancelled by a goto matker should be dashed, and should not afftect
  //         the layout of the graph [constraint=false]
};
