...
BOOST_AUTO_TEST_CASE(PrimitiveTypes) {
  revng_check(PrimitiveType("uint8_t").verify());
  revng_check(PrimitiveType("uint16_t").verify());
  revng_check(PrimitiveType("uint32_t").verify());
  revng_check(PrimitiveType("uint64_t").verify());
  revng_check(PrimitiveType("uint128_t").verify());
  revng_check(PrimitiveType("int8_t").verify());
  revng_check(PrimitiveType("int16_t").verify());
  revng_check(PrimitiveType("int32_t").verify());
  revng_check(PrimitiveType("int64_t").verify());
  revng_check(PrimitiveType("int128_t").verify());
  revng_check(PrimitiveType("bool8_t").verify());
  revng_check(PrimitiveType("bool16_t").verify());
  revng_check(PrimitiveType("bool32_t").verify());
  revng_check(PrimitiveType("bool64_t").verify());
  revng_check(PrimitiveType("bool128_t").verify());
  revng_check(PrimitiveType("float16_t").verify());
  revng_check(PrimitiveType("float32_t").verify());
  revng_check(PrimitiveType("float64_t").verify());
  revng_check(PrimitiveType("float80_t").verify());
  revng_check(PrimitiveType("float128_t").verify());
  revng_check(PrimitiveType("void").verify());

  revng_check(not PrimitiveType().verify());
  revng_check(not PrimitiveType("char").verify());
  revng_check(not PrimitiveType("unsigned").verify());
  revng_check(not PrimitiveType("signed").verify());
  revng_check(not PrimitiveType("unsigned char").verify());
  revng_check(not PrimitiveType("signed char").verify());
  revng_check(not PrimitiveType("short").verify());
  revng_check(not PrimitiveType("int").verify());
  revng_check(not PrimitiveType("long").verify());
  revng_check(not PrimitiveType("long long").verify());
  revng_check(not PrimitiveType("unsigned short").verify());
  revng_check(not PrimitiveType("unsigned int").verify());
  revng_check(not PrimitiveType("unsigned long").verify());
  revng_check(not PrimitiveType("unsigned long long").verify());
  revng_check(not PrimitiveType("size_t").verify());
  revng_check(not PrimitiveType("ssize_t").verify());
  revng_check(not PrimitiveType("uintptr_t").verify());
  revng_check(not PrimitiveType("intptr_t").verify());
  revng_check(not PrimitiveType("bool").verify());
  revng_check(not PrimitiveType("float").verify());
  revng_check(not PrimitiveType("double").verify());
  revng_check(not PrimitiveType("long double").verify());
  revng_check(not PrimitiveType("randomname").verify());
}
...
BOOST_AUTO_TEST_CASE(EnumTypes) {
  ...
  {
    const auto &[It, New] = T->Types.insert(makeType<PrimitiveType>("int32_t"));
    revng_check(New);
...
BOOST_AUTO_TEST_CASE(StructTypes) {
  ...
  // Insert void type
  PrimitiveType *Void = nullptr;
  {
    const auto &[It, New] = T->Types.insert(makeType<PrimitiveType>("void"));
    revng_check(New);
    revng_check(T->Types.size() == 2);
    Void = cast<PrimitiveType>(T->Types.at((*It)->key()).get());
    revng_check(Void and Void->verify());
    revng_check(T->verify());
    revng_check(checkSerialization(T));
  }
  ...
  // Shrinking the size fixes the struct
  Struct->Size = 132;
  revng_check(Struct->verify());
  revng_check(T->verify());
  revng_check(checkSerialization(T));
...
BOOST_AUTO_TEST_CASE(UnionTypes) {
  ...
  // Adding a void field is not valid
  Union->Fields.clear();
  UnionField VoidField = UnionField{ { makeTypePath(Void), {} }, "void_fld" };
...
BOOST_AUTO_TEST_CASE(FunctionPointerTypes) {
  ...
  // an ArgumentType cannot have the same type as the function type
  ...
  // an ArgumentType cannot return the same type as the function type
